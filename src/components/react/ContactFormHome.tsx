...
export default function ContactFormHome(props: ContactFormHomeProps) {
  ...
  const contactFormHomeData = props.contactFormHome.filter(
        ...
        return (
          <div key={index}>
            <div className="flex flex-col sm:mb-32">
              <h2 className="text-text text-6xl sm:text-9xl font-bold">
                {data.title}
              </h2>
              <span className="text-right text-4xl md:text-5xl font-thin text-text">
                {data.subtitle}
              </span>
            </div>
